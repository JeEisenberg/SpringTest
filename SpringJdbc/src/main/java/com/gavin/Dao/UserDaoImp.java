...
public class UserDaoImp implements UserDao {
    ...
    public void addUser(User user) {
        String sql = "insert into test values(" + user.getId() + "," + user.getName() + "," + user.getAge() + ")";
...
@Transactional(transactionManager = "transactionManager" ,propagation = Propagation.REQUIRED,
    ...
    public void transfer(String out, String in, Integer jf) {
        ...
        int update = jdbcTemplate.update(sql,out,jf);
        String sql2="update test set score= score + ? where name=?";
        int update2 = jdbcTemplate.update(sql2,in,jf);
